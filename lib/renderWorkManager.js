...
function work(manager) {
    ...
    var jobChunkIterator = makeChunkIterator(job.width, job.height, job.json, ConnectionManager.getNumConnections());
    ...
    while (!jobChunk.done) {
        ...
        if (freeWorker) {
            ...
            freeWorker.socket.emit('worker-job', jobChunk.value);
        }
