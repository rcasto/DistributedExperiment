...
function RenderWorkManager() {
    ...
    this.on('worker-done', function (result) {
        ...
        this.activeWorkers.delete(result.socket.id);
        ...
        if (this.activeWorkers.size === 0 && this.unfinishedWork.length === 0) {
            ...
            if (this.unfinishedWork.length > 0) {
                ...
                this.jobMap.set(worker.socket.id, job);
                this.activeWorkers.set(worker.socket.id, worker);
            }
        }
    }.bind(this));
}
...
function findWorker(element, index, array) {
...
function work(manager) {
    ...
    for (var workerIndex = 0; (workerIndex < numWorkers) && !jobChunk.done; workerIndex++) {
        ...
        manager.jobMap.set(workers[workerIndex].socket.id, jobChunk);
        manager.activeWorkers.set(workers[workerIndex].socket.id, workers[workerIndex]);
